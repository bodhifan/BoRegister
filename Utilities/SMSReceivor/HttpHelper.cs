...
namespace Utilities
{
    ...
    public class HttpHelper
    {
            ...
            try
            {
                ...
                using (response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    using (MemoryStream _stream = new MemoryStream())
                    {
                        ...
                        {
                            //开始读取流并设置编码方式
                            new GZipStream(response.GetResponseStream(), CompressionMode.Decompress).Read(bufBytes,0,10240);
                            _stream.Write(bufBytes, 0, 10240);
                           //     .CopyTo(_stream, 10240);
                        }
                        else
                        {
                            //开始读取流并设置编码方式
                            response.GetResponseStream().Read(bufBytes,0,10240);
                             _stream.Write(bufBytes, 0, 10240);
                            //    .CopyTo(_stream, 10240);
                        }
                        //获取Byte
        ...
        private void SetRequest(HttpItem item)
        {
            ...
            if (!IsNullOrWhiteSpace(item.Host))
            {
                //request. = item.Host;
            }
            //Accept
        ...
        {
            if (!IsNullOrWhiteSpace(item.CerPath))
            {
        ...
        private void SetPostData(HttpItem item)
        {
            ...
            if (request.Method.Trim().ToLower().Contains("post"))
            {
                ...
                else if (item.PostDataType == PostDataType.FilePath && !IsNullOrWhiteSpace(item.Postdata))
                {
                    StreamReader r = new StreamReader(item.Postdata, postencoding);
                    buffer = postencoding.GetBytes(r.ReadToEnd());
                    r.Close();
                } //写入字符串
                else if (!IsNullOrWhiteSpace(item.Postdata))
                {
        ...
        {
            if (!IsNullOrWhiteSpace(item.ProxyIp))
            {
                //设置代理服务器
